/**
 * End-to-end example: Verify commitment-based Primus attestation on local sandbox
 * 
 * This script demonstrates the complete workflow on a local Aztec sandbox:
 * 1. Start local PXE (PXE_PROVER_ENABLED=1 aztec start --local-network)
 * 2. Initialize Aztec client
 * 3. Parse attestation data
 * 4. Deploy verifier contract
 * 5. Call verify_comm function
 * 6. Wait for success event
 * 
 * Prerequisites:
 * - Aztec Sandbox running: PXE_PROVER_ENABLED=1 aztec start --local-network
...
async function main() {
  console.log('🚀 Starting commitment-based attestation verification on local sandbox...\n');

  // Step 1: Initialize client (local mode)
  console.log('📡 Initializing Aztec client (local mode)...');
  ...
  // Step 2: Get account (instant in local mode)
  ...
  // Step 4: Deploy contract (no fees in local mode)
  ...
  // Step 5: Verify attestation (no fees in local mode)
  ...
  const result = await contract.methods
    ...
    .send({ from: alice.address })  // No fee needed in local mode
    .wait({ timeout: 180000 });
